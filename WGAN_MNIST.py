...
def smaple_image(imgtag):
    row = 10
    z = torch.randn((row**2,opt.latent_dim), device=device)
    generator.eval()
    with torch.inference_mode():
        sample = generator(z)
    save_image(sample,f'./samples/WGAN_{imgtag}.png',nrow=row ,normalize=True)
    generator.train()
...
dataloader = DataLoader(dataset_1, batch_size=opt.batch_size, shuffle=True,num_workers=0)

#Train D/G ration = 5:1
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, _) in enumerate(dataloader):
        ...
        z = torch.randn((imgs.size(0), opt.latent_dim), device=device)#(-∞, ∞)正態
        ...
        for p in discriminator.parameters():
            p.data.clamp_(-opt.clip_value, opt.clip_value) #就地操作:把參數值限制在給定範圍+-0.01內
    ...
    print(f"[Epoch {epoch:03d}/{opt.n_epochs:03d}][D loss: {loss_D.item():.4f}] [G loss: {loss_G.item():.4f}]--{time1round}")    
    if epoch %50 == 0 or epoch == 199:
        smaple_image(imgtag=epoch)

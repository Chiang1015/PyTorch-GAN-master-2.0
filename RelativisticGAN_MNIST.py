...
#修改布林選項action='store_true'
parser.add_argument("--rel_avg_gan", type=bool,default=True, help="relativistic average GAN instead of standard")
...
def smaple_images(imgtag):
    row = 10
    z = torch.empty((row**2, opt.latent_dim), device=device).uniform_(-1, 1)
    generator.eval()
    with torch.inference_mode():
        sample = generator(z)
    save_image(sample,f'./samples/RelativeGAN_{imgtag}.png',nrow=row,normalize=True)
    generator.train()

#input: (b,100) => img(b,3,32,32)
class Generator(nn.Module):
    ...
    def forward(self, z):
        ...
        return img #(b,3,32,32)

#input:(b,3,32,32) => val(b,1)
